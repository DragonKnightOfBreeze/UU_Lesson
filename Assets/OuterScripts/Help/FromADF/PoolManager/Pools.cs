...
namespace Kernel {
	public class Pools : MonoBehaviour {
		...
		public bool IsUsedTime;                                              //是否用“时间戳”
		...
		/// <summary>
		///     时间戳处理 主要业务逻辑: 1>： 每间隔10秒种，对所有正在使用的活动状态游戏对象的时间戳减去10秒。 2>:  检查每个活动状态的游戏对象名称的时间戳如果小于等于0，则进入禁用状态。 3>:
		///     重新进入活动状态的游戏对象，获得预先设定的存活时间写入对象名称的时间戳中。
		/// </summary>
		private void ProcessGameObject_NameTime() {
			//循环体为定义的“种类”数量
			for(var i = 0; i < PoolOptionArrayLib.Count; i++) {
				var opt = PoolOptionArrayLib[i];
		...
		public void PreLoadGameObject() {
			for(var i = 0; i < PoolOptionArrayLib.Count; i++) { //“多模”集合
				var opt = PoolOptionArrayLib[i];                //“单模”集合
				for(var j = opt.totalCount; j < opt.IntPreLoadNumber; j++) {
					var obj = opt.PreLoad(opt.Prefab, Vector3.zero, Quaternion.identity);
		...
		/// <summary>得到游戏对象（“多模”集合） 功能描述： 1： 对指定“预设”在自己的缓冲池中激活一个，且加入自己缓冲池中的"可用激活池"。 2： 然后再建立一个池对象，且激活预设，再加入自己的缓冲池中的“可用激活池”中。</summary>
		...
		/// <summary>当本脚本所挂载的游戏对象销毁时候，清空所有集合内全部数据</summary>
	...
	/// <summary>游戏（单类型）缓冲对象管理（即：单模池操作管理） 功能： 激活、收回、预加载等。</summary>
	[System.Serializable]
	public class PoolOption {
		public GameObject Prefab;                        //存储的“预设”
		public int IntPreLoadNumber;                     //初始缓冲数量
		...
		internal GameObject PreLoad(GameObject prefab, Vector3 positon, Quaternion rotation) {
			...
			if(prefab) {
				obj = Object.Instantiate(prefab, positon, rotation);
				Rename(obj);
		...
		/// <summary>统计两个“池”中所有对象的数量</summary>
		internal int totalCount {
			get {
				var count = 0;
				count += ActiveGameObjectArray.Count;
				count += InactiveGameObjectArray.Count;
				return count;
			}
		}

		/// <summary>全部清空集合（两个“池”）</summary>
		internal void ClearAllArray() {
			ActiveGameObjectArray.Clear();
			InactiveGameObjectArray.Clear();
		}

		/// <summary>彻底删除所有“非活动”集合容器中的游戏对象。</summary>
		internal void ClearUpUnused() {
			foreach(var obj in InactiveGameObjectArray)
				Object.Destroy(obj);

			InactiveGameObjectArray.Clear();
		}

		/// <summary>游戏对象重命名 对新产生的游戏对象做统一格式处理，目的是做“时间戳”处理。</summary>
		/// <param name="instance"></param>
		...
		/// <summary>删除“非活动”容器集合中的一部分指定数量数据</summary>
		/// <param name="count"></param>
		internal void DestoryCount(int count) {
			...
			for(var i = InactiveGameObjectArray.Count - 1; i >= InactiveGameObjectArray.Count - count; i--)
				Object.Destroy(InactiveGameObjectArray[i]);
			InactiveGameObjectArray.RemoveRange(InactiveGameObjectArray.Count - count, count);
		}

		/// <summary>回调函数：时间戳管理 功能：所有游戏对象进行时间倒计时管理，时间小于零则进行“非活动”容器集合中，即:按时间自动回收游戏对象。</summary>
		internal void AllActiveGameObjectTimeSubtraction() {
			for(var i = 0; i < ActiveGameObjectArray.Count; i++) {
				string strHead = null;
				string strTail = null;
				var intTimeInfo = 0;
				...
				//得到每个对象的时间戳
				var strArray = goActiveObj.name.Split('@');
				strHead = strArray[0];
				strTail = strArray[1];
				...
				if(intTimeInfo >= 10) {
					...
					//游戏对象自动转入禁用
					goActiveObj.name = IntAutoDeactiveGameObjectByTime + "@" + strTail;
					Deactive(goActiveObj);
					continue;
				}
				//时间戳重新生成
