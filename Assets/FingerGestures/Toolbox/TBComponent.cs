...
/// <summary>
///     Base class for all the TB* gesture components (TBDrag, TBTap, TBLongPress, TBSwipe...). Implements commonly
///     use methods and data structures. NOTE: the GameObject must have a collider (it's used by the TBInputManager when
///     raycasting into the scene to find the object under the finger). Currently, this won't work out of the box with
///     GUIText objects, due to the way they are behind rendered (2D). It will work with a 3D TextMesh though.
/// </summary>
public abstract class TBComponent : MonoBehaviour {
	...
	protected virtual void Start() {
		if(!GetComponent<Collider>()) {
			Debug.LogError(name + " must have a valid collider.");
			enabled = false;
		}
	}
