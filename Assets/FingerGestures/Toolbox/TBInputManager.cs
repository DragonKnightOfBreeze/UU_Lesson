...
/// <summary>
///     Toolbox InputManager This class acts as a HUB/manager for the various input gesture events. It dispatches
///     calls to the various TB* classes, such as TBDrag, TBTap, etc... There should be exactly one instance of this class
///     in the scene. The set of toolbox scripts currently supports: - FingerDown - FingerUp - Drag & Drop - Tap -
///     Long-Press - Swipe Please check the ToolboxDragDrop sample scenes for a solid example on how to use this system. -
///     Drag & Drop Implementation Details - This class listens to the FingerGestures.OnFingerDragBegin event and raycasts
///     into the scene at the given finger position in order to find a valid object equipped with a TBDrag component. If it
///     finds one, it calls TBDrag.BeginDrag() on this object, and starts listening to TBDrag.OnDragMove and
///     TBDrag.OnDragEnd. The TBDrag object position is updated in dragged_OnDragMove.
/// </summary>
...
public class TBInputManager : MonoBehaviour {
	...
	private void FingerGestures_OnFingerUp(int fingerIndex, Vector2 fingerPos, float timeHeldDown) {
		var fingerUpComp = PickComponent<TBFingerUp>(fingerPos);
		if(fingerUpComp)
	...
	private void FingerGestures_OnFingerDown(int fingerIndex, Vector2 fingerPos) {
		var fingerDownComp = PickComponent<TBFingerDown>(fingerPos);
		if(fingerDownComp)
	...
	public float dragPlaneOffset;                              // distance between dragged object and drag constraint plane
	...
	private void FingerGestures_OnFingerDragBegin(int fingerIndex, Vector2 fingerPos, Vector2 startPos) {
		...
		var draggable = PickComponent<TBDrag>(startPos);
	...
	private bool ProjectScreenPointOnDragPlane(Vector3 refPos, Vector2 screenPos, out Vector3 worldPos) {
		...
		switch(dragPlaneType) {
			...
			case DragPlaneType.Camera: {
				var camTransform = raycastCamera.transform;

				// create a plane passing through refPos and facing toward the camera
				var plane = new Plane(-camTransform.forward, refPos);
	...
	private void draggable_OnDragMove(TBDrag sender) {
		...
		var prevFingerPos = sender.FingerPos - sender.MoveDelta;
		...
		if(ProjectScreenPointOnDragPlane(sender.transform.position, prevFingerPos, out prevFingerPos3d) &&
		   ProjectScreenPointOnDragPlane(sender.transform.position, sender.FingerPos, out fingerPos3d)) {
			var move = fingerPos3d - prevFingerPos3d;
	...
	private void FingerGestures_OnFingerTap(int fingerIndex, Vector2 fingerPos, int tapCount) {
		var tapComp = PickComponent<TBTap>(fingerPos);
		if(tapComp)
	...
	private void FingerGestures_OnFingerLongPress(int fingerIndex, Vector2 fingerPos) {
		var longPressComp = PickComponent<TBLongPress>(fingerPos);
		if(longPressComp)
	...
	private void FingerGestures_OnFingerSwipe(int fingerIndex, Vector2 startPos, FingerGestures.SwipeDirection direction, float velocity) {
		var swipeComp = PickComponent<TBSwipe>(startPos);
		if(swipeComp)
