...
///     level. All base items are created with their stats at max level. Game item, the second item class, has an effective
///     item level which is used to calculate effective item stats. Game items can be generated with a random level
///     (clamped within base item's min/max level range), and with random quality affecting the item's stats.
/// </summary>
[System.Serializable]
public class InvGameItem {
	...
	/// <summary>Item quality -- applies a penalty or bonus to all base stats.</summary>
	public Quality quality = Quality.Sturdy;

	/// <summary>Item's effective level.</summary>
	public int itemLevel = 1;

	// Cached for speed
	private InvBaseItem mBaseItem;

	/// <summary>ID of the base item used to create this one.</summary>

	public int baseItemID => mBaseItemID;

	/// <summary>Base item used by this game item.</summary>
	...
	/// <summary>Game item's name should prefix the quality</summary>
	...
	/// <summary>
	///     Put your formula for calculating the item stat modifier here. Simplest formula -- scale it with quality and
	///     item level. Since all stats on base items are specified at max item level, calculating the effective multiplier is
	///     as simple as itemLevel/maxLevel.
	/// </summary>
	...
	/// <summary>Item's color based on quality. You will likely want to change this to your own colors.</summary>
	...
	/// <summary>Create a game item with the specified ID.</summary>
	public InvGameItem(int id) {
		mBaseItemID = id;
	}

	/// <summary>Create a game item with the specified ID and base item.</summary>
	public InvGameItem(int id, InvBaseItem bi) {
		mBaseItemID = id;
		mBaseItem = bi;
	}

	/// <summary>Calculate and return the list of effective stats based on item level and quality.</summary>
	public List<InvStat> CalculateStats() {
		var stats = new List<InvStat>();

		if(baseItem != null) {
			var mult = statMultiplier;
			var baseStats = baseItem.stats;

			for(int i = 0, imax = baseStats.Count; i < imax; ++i) {
				var bs = baseStats[i];
				var amount = Mathf.RoundToInt(mult * bs.amount);
