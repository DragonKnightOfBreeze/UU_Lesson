...
/// <summary>
///     Inventory system -- Equipment class works with InvAttachmentPoints and allows to visually equip and remove
///     items.
/// </summary>
...
public class InvEquipment : MonoBehaviour {
	...
	/// <summary>List of equipped items (with a finite number of equipment slots).</summary>

	public InvGameItem[] equippedItems => mItems;

	/// <summary>Equip the specified item automatically replacing an existing one.</summary>
	public InvGameItem Replace(InvBaseItem.Slot slot, InvGameItem item) {
		...
		if(slot != InvBaseItem.Slot.None) {
			...
			if(mItems == null) {
				...
				var count = (int) InvBaseItem.Slot._LastDoNotUse;
				mItems = new InvGameItem[count];
			}

			// Equip this item
			var prev = mItems[(int) slot - 1];
			...
			for(int i = 0, imax = mAttachments.Length; i < imax; ++i) {
				...
				if(ip.slot == slot) {
					var go = ip.Attach(baseItem != null ? baseItem.attachment : null);
	...
	/// <summary>Equip the specified item and return the item that was replaced.</summary>
	...
	/// <summary>Unequip the specified item, returning it if the operation was successful.</summary>
	...
	/// <summary>Unequip the item from the specified slot, returning the item that was unequipped.</summary>
	public InvGameItem Unequip(InvBaseItem.Slot slot) {
		return Replace(slot, null);
	}

	/// <summary>Whether the specified item is currently equipped.</summary>
	...
	/// <summary>Whether the specified slot currently has an item equipped.</summary>
	public bool HasEquipped(InvBaseItem.Slot slot) {
		if(mItems != null)
			for(int i = 0, imax = mItems.Length; i < imax; ++i) {
				var baseItem = mItems[i].baseItem;
				if(baseItem != null && baseItem.slot == slot) return true;
			}
		return false;
	}

	/// <summary>Retrieves the item in the specified slot.</summary>
