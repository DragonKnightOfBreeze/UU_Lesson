...
/// <summary>
///     Attach this script to any object that has idle animations. It's expected that the main idle loop animation is
///     called "idle", and idle break animations all begin with "idle" (ex: idleStretch, idleYawn, etc). The script will
///     place the idle loop animation on layer 0, and breaks on layer 1.
/// </summary>
[AddComponentMenu("NGUI/Examples/Play Idle Animations")]
public class PlayIdleAnimations : MonoBehaviour {
	private Animation mAnim;
	private AnimationClip mIdle;
	private readonly List<AnimationClip> mBreaks = new List<AnimationClip>();
	private float mNextBreak;
	private int mLastIndex;

	/// <summary>Find all idle animations.</summary>
	private void Start() {
	...
	/// <summary>If it's time to play a new idle break animation, do so.</summary>
	private void Update() {
		if(mNextBreak < Time.time) {
			if(mBreaks.Count == 1) {
				...
				var index = Random.Range(0, mBreaks.Count - 1);
