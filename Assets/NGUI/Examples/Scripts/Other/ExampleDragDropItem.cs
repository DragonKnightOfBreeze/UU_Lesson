...
public class ExampleDragDropItem : UIDragDropItem {
	/// <summary>Prefab object that will be instantiated on the DragDropSurface if it receives the OnDrop event.</summary>
	public GameObject prefab;

	/// <summary>Drop a 3D game object onto the surface.</summary>
	protected override void OnDragDropRelease(GameObject surface) {
		if(surface != null) {
			var dds = surface.GetComponent<ExampleDragDropSurface>();

			if(dds != null) {
				var child = dds.gameObject.AddChild(prefab);
