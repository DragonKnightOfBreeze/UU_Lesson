...
/// <summary>UI Atlas contains a collection of sprites inside one large texture atlas.</summary>
...
public class UIAtlas : MonoBehaviour {
	...
	/// <summary>Legacy functionality, removed in 3.0. Do not use.</summary>
	...
	private readonly Dictionary<string, int> mSpriteIndices = new Dictionary<string, int>();

	/// <summary>Material used by the atlas.</summary>

	public Material spriteMaterial {
		get { return mReplacement != null ? mReplacement.spriteMaterial : material; }
		set {
	...
	/// <summary>Whether the atlas is using a premultiplied alpha material.</summary>
	...
	/// <summary>List of sprites within the atlas.</summary>
	...
	/// <summary>Texture used by the atlas.</summary>

	public Texture texture => mReplacement != null ? mReplacement.texture : material != null ? material.mainTexture : null;

	/// <summary>
	///     Pixel size is a multiplier applied to widgets dimensions when performing MakePixelPerfect() pixel correction.
	///     Most obvious use would be on retina screen displays. The resolution doubles, but with UIRoot staying the same for
	///     layout purposes, you can still get extra sharpness by switching to an HD atlas that has pixel size set to 0.5.
	/// </summary>
	...
	public float pixelSize {
		...
		set {
			if(mReplacement != null) {
				...
				var val = Mathf.Clamp(value, 0.25f, 4f);
	...
	///     Suggested use: set up all your widgets to use a dummy atlas that points to the real atlas. Switching that atlas to
	...
	/// <summary>Convenience function that retrieves a sprite by name.</summary>
	...
	/// <summary>Convenience function that returns the name of a random sprite that begins with the specified value.</summary>
	public string GetRandomSprite(string startsWith) {
		if(GetSprite(startsWith) == null) {
			var sprites = spriteList;
			var choices = new List<string>();

			foreach(var sd in sprites)
	...
	/// <summary>Rebuild the sprite indices. Call this after modifying the spriteList at run time.</summary>
	...
	/// <summary>Sort the list of sprites within the atlas, making them alphabetical.</summary>
	...
	/// <summary>Convenience function that retrieves a list of all sprite names.</summary>
	...
	/// <summary>Convenience function that retrieves a list of all sprite names that contain the specified phrase</summary>
	public BetterList<string> GetListOfSprites(string match) {
		...
		var keywords = match.Split(new[] {' '}, StringSplitOptions.RemoveEmptyEntries);
	...
	/// <summary>Helper function that determines whether the atlas uses the specified one, taking replacements into account.</summary>
	...
	/// <summary>Mark all widgets associated with this atlas as having changed.</summary>
	...
	/// <summary>Performs an upgrade from the legacy way of specifying data to the new one.</summary>
	private bool Upgrade() {
		if(mReplacement) return mReplacement.Upgrade();

		if(mSprites.Count == 0 && sprites.Count > 0 && material) {
			var tex = material.mainTexture;
			var width = tex != null ? tex.width : 512;
			var height = tex != null ? tex.height : 512;
