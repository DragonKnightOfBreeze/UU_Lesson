...
/// <summary>Example script that can be used to show tooltips.</summary>
...
public class UITooltip : MonoBehaviour {
	...
	/// <summary>Get a list of widgets underneath the tooltip.</summary>
	...
	/// <summary>Update the tooltip's alpha based on the target value.</summary>
	protected virtual void Update() {
		...
		if(mCurrent != mTarget) {
			...
			if(scalingTransitions) {
				var offset = mSize * 0.25f;
				offset.y = -offset.y;

				var size = Vector3.one * (1.5f - mCurrent * 0.5f);
				var pos = Vector3.Lerp(mPos - offset, mPos, mCurrent);
	...
	/// <summary>Set the alpha of all widgets.</summary>
	...
	/// <summary>Set the tooltip's text to the specified string.</summary>
	protected virtual void SetText(string tooltipText) {
		if(text != null && !string.IsNullOrEmpty(tooltipText)) {
			...
			var offset = textTrans.localPosition;
			var textScale = textTrans.localScale;
			...
			if(uiCamera != null) {
				...
				var activeSize = uiCamera.orthographicSize / mTrans.parent.lossyScale.y;
				var ratio = Screen.height * 0.5f / activeSize;

				// Calculate the maximum on-screen size of the tooltip window
				var max = new Vector2(ratio * mSize.x / Screen.width, ratio * mSize.y / Screen.height);
	...
	/// <summary>Show a tooltip with the specified text.</summary>
