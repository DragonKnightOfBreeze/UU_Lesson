...
/// <summary>Popup list can be used to display pop-up menus and drop-down lists.</summary>
[ExecuteInEditMode]
[AddComponentMenu("NGUI/Interaction/Popup List")]
public class UIPopupList : UIWidgetContainer {
	/// <summary>Current popup list. Only available during the OnSelectionChange event callback.</summary>
	...
	/// <summary>Atlas used by the sprites.</summary>
	public UIAtlas atlas;

	/// <summary>Font used by the labels.</summary>
	public UIFont bitmapFont;

	/// <summary>True type font used by the labels. Alternative to specifying a bitmap font ('font').</summary>
	public Font trueTypeFont;

	/// <summary>Font used by the popup list. Conveniently wraps both dynamic and bitmap fonts into one property.</summary>
	...
	/// <summary>Size of the font to use for the popup list's labels.</summary>
	public int fontSize = 16;

	/// <summary>Font style used by the dynamic font.</summary>
	public FontStyle fontStyle = FontStyle.Normal;

	/// <summary>Name of the sprite used to create the popup's background.</summary>
	public string backgroundSprite;

	/// <summary>Name of the sprite used to highlight items.</summary>
	...
	/// <summary>Popup list's display style.</summary>
	public Position position = Position.Auto;

	/// <summary>Whether the popup item selection is chosen by pressing on it or by clicking on it.</summary>
	public Selection selection = Selection.OnPress;

	/// <summary>Label alignment to use.</summary>
	public NGUIText.Alignment alignment = NGUIText.Alignment.Left;

	/// <summary>New line-delimited list of items.</summary>
	public List<string> items = new List<string>();

	/// <summary>
	///     You can associate arbitrary data to be associated with your entries if you like. The only downside is that
	///     this must be done via code.
	/// </summary>
	public List<object> itemData = new List<object>();

	/// <summary>Amount of padding added to labels.</summary>
	public Vector2 padding = new Vector3(4f, 4f);

	/// <summary>Color tint applied to labels inside the list.</summary>
	public Color textColor = Color.white;

	/// <summary>Color tint applied to the background.</summary>
	public Color backgroundColor = Color.white;

	/// <summary>Color tint applied to the highlighter.</summary>
	public Color highlightColor = new Color(225f / 255f, 200f / 255f, 150f / 255f, 1f);

	/// <summary>Whether the popup list is animated or not. Disable for better performance.</summary>
	public bool isAnimated = true;

	/// <summary>Whether the popup list's values will be localized.</summary>
	public bool isLocalized;

	/// <summary>Custom text modifier for child labels.</summary>
	public UILabel.Modifier textModifier = UILabel.Modifier.None;

	/// <summary>Whether a separate panel will be used to ensure that the popup will appear on top of everything else.</summary>
	public bool separatePanel = true;

	/// <summary>Amount by which the popup's border will overlap with the content that opened it.</summary>
	public int overlap;
	...
	/// <summary>What kind of click is needed in order to open the popup list.</summary>
	public OpenOn openOn = OpenOn.ClickOrTap;

	/// <summary>Callbacks triggered when the popup list gets a new item selection.</summary>
	...
	[HideInInspector]
	[SerializeField]
	private UIFont font; // Use 'bitmapFont' instead
	...
	public LegacyEvent onSelectionChange {
		get { return mLegacyEvent; }
		set { mLegacyEvent = value; }
	}

	/// <summary>Whether the popup list is currently open.</summary>
	...
	/// <summary>Item data associated with the current selection.</summary>

	public virtual object data {
		get {
			var index = items.IndexOf(mSelectedItem);
	...
	/// <summary>Whether the popup list is actually usable.</summary>

	protected bool isValid => bitmapFont != null || trueTypeFont != null;

	/// <summary>Active font size.</summary>

	protected int activeFontSize => trueTypeFont != null || bitmapFont == null ? fontSize : bitmapFont.defaultSize;

	/// <summary>Font scale applied to the popup list's text.</summary>

	protected float activeFontScale => trueTypeFont != null || bitmapFont == null ? 1f : (float) fontSize / bitmapFont.defaultSize;

	/// <summary>Set the current selection.</summary>
	...
	/// <summary>Clear the popup list's contents.</summary>
	public virtual void Clear() {
		items.Clear();
		itemData.Clear();
	}

	/// <summary>Add a new item to the popup list.</summary>
	...
	/// <summary>Remove the specified item.</summary>
	...
	public virtual void RemoveItemByData(object data) {
		var index = itemData.IndexOf(data);
	...
	/// <summary>Trigger all event notification callbacks.</summary>
	...
	/// <summary>Remove legacy functionality.</summary>
	...
	/// <summary>Send out the selection message on start.</summary>
	...
	/// <summary>Localize the text label.</summary>
	protected virtual void OnLocalize() {
		if(isLocalized) TriggerCallbacks();
	}

	/// <summary>Visibly highlight the specified transform by moving the highlight sprite to be over it.</summary>
	protected virtual void Highlight(UILabel lbl, bool instant) {
		if(mHighlight != null) {
			mHighlightedLabel = lbl;

			var pos = GetHighlightPosition();
	...
	/// <summary>Helper function that calculates where the tweened position should be.</summary>
	protected virtual Vector3 GetHighlightPosition() {
		...
		var scaleFactor = atlas != null ? atlas.pixelSize : 1f;
		var offsetX = border.x * scaleFactor;
		var offsetY = border.w * scaleFactor;
	...
	/// <summary>
	///     Periodically update the tweened target position. It's needed because the popup list animates into view, and
	...
	/// <summary>Event function triggered when the mouse hovers over an item.</summary>
	protected virtual void OnItemHover(GameObject go, bool isOver) {
		if(isOver) {
			var lbl = go.GetComponent<UILabel>();
			Highlight(lbl, false);
		}
	}

	/// <summary>Event function triggered when the drop-down list item gets pressed on.</summary>
	protected virtual void OnItemPress(GameObject go, bool isPressed) {
		if(isPressed && selection == Selection.OnPress) OnItemClick(go);
	}

	/// <summary>Event function triggered when the drop-down list item gets clicked on.</summary>
	protected virtual void OnItemClick(GameObject go) {
		...
		var sounds = GetComponents<UIPlaySound>();
	...
	/// <summary>Select the specified label.</summary>
	private void Select(UILabel lbl, bool instant) {
		Highlight(lbl, instant);
	}

	/// <summary>React to key-based input.</summary>
	...
	/// <summary>Close the popup list when disabled.</summary>
	protected virtual void OnDisable() {
		CloseSelf();
	}

	/// <summary>Get rid of the popup dialog when the selection gets lost.</summary>
	...
	/// <summary>Manually close the popup list.</summary>
	...
	public virtual void CloseSelf() {
		if(mChild != null && current == this) {
			...
			if(isAnimated) {
				var widgets = mChild.GetComponentsInChildren<UIWidget>();
				...
				var cols = mChild.GetComponentsInChildren<Collider>();
	...
	/// <summary>Helper function that causes the widget to smoothly fade in.</summary>
	...
	/// <summary>Helper function that causes the widget to smoothly move into position.</summary>
	protected virtual void AnimatePosition(UIWidget widget, bool placeAbove, float bottom) {
		var target = widget.cachedTransform.localPosition;
		var start = placeAbove ? new Vector3(target.x, bottom, target.z) : new Vector3(target.x, 0f, target.z);
	...
	/// <summary>Helper function that causes the widget to smoothly grow until it reaches its original size.</summary>
	protected virtual void AnimateScale(UIWidget widget, bool placeAbove, float bottom) {
		var go = widget.gameObject;
		var t = widget.cachedTransform;
	...
	/// <summary>Helper function used to animate widgets.</summary>
	protected void Animate(UIWidget widget, bool placeAbove, float bottom) {
		AnimateColor(widget);
		AnimatePosition(widget, placeAbove, bottom);
	}

	/// <summary>Display the drop-down list when the game object gets clicked on.</summary>
	...
	/// <summary>Show the popup list on double-click.</summary>
	protected virtual void OnDoubleClick() {
		if(openOn == OpenOn.DoubleClick) Show();
	}

	/// <summary>Used to keep an eye on the selected object, closing the popup if it changes.</summary>
	...
	/// <summary>Show the popup list dialog.</summary>
	public virtual void Show() {
		if(enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0) {
			...
			if(openOn == OpenOn.Manual && mSelection != gameObject) {
				...
				var bounds = NGUIMath.CalculateRelativeWidgetBounds(pTrans, transform, false, false);
				min = bounds.min;
				max = bounds.max;
				t.localPosition = min;
				startingPosition = t.position;
			...
			if(background2DSprite != null) {
				...
				mBackground = mChild.AddSprite(atlas, backgroundSprite, depth);
			}
			else {
				return;
			}
			...
			if(highlight2DSprite != null) {
				...
				mHighlight = mChild.AddSprite(atlas, highlightSprite, ++depth);
			}
			else {
				return;
			}
			...
			for(int i = 0, imax = items.Count; i < imax; ++i) {
				...
				var lbl = mChild.AddWidget<UILabel>(mBackground.depth + 2);
				lbl.name = i.ToString();
