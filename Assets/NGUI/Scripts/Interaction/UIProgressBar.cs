...
/// <summary>Simple progress bar that fills itself based on the specified value.</summary>
[ExecuteInEditMode]
...
public class UIProgressBar : UIWidgetContainer {
	...
	/// <summary>Current slider. This value is set prior to the callback function being triggered.</summary>
	public static UIProgressBar current;

	/// <summary>
	///     Delegate triggered when the scroll bar stops being dragged. Useful for things like centering on the closest
	...
	/// <summary>Object that acts as a thumb.</summary>
	...
	/// <summary>
	///     Number of steps the slider should be divided into. For example 5 means possible values of 0, 0.25, 0.5, 0.75,
	///     and 1.0.
	/// </summary>
	public int numberOfSteps;

	/// <summary>Callbacks triggered when the scroll bar's value changes.</summary>
	public List<EventDelegate> onChange = new List<EventDelegate>();

	/// <summary>Cached for speed.</summary>
	...
	/// <summary>Camera used to draw the scroll bar.</summary>
	...
	/// <summary>Widget used for the foreground.</summary>
	...
	/// <summary>Widget used for the background.</summary>
	...
	/// <summary>The scroll bar's direction.</summary>
	...
	/// <summary>Modifiable value for the scroll bar, 0-1 range.</summary>
	...
	/// <summary>
	///     Allows to easily change the scroll bar's alpha, affecting both the foreground and the background sprite at
	///     once.
	/// </summary>
	...
	public float alpha {
		...
		set {
			...
			if(thumb != null) {
				var w = thumb.GetComponent<UIWidget>();
	...
	/// <summary>Whether the progress bar is horizontal in nature. Convenience function.</summary>

	protected bool isHorizontal => mFill == FillDirection.LeftToRight || mFill == FillDirection.RightToLeft;

	/// <summary>Whether the progress bar is inverted in its behaviour. Convenience function.</summary>

	protected bool isInverted => mFill == FillDirection.RightToLeft || mFill == FillDirection.TopToBottom;

	/// <summary>Set the progress bar's value. If setting the initial value, call Start() first.</summary>
	...
	/// <summary>Register the event listeners.</summary>
	public void Start() {
		if(mStarted) return;
		mStarted = true;
		Upgrade();
	...
	/// <summary>Used to upgrade from legacy functionality.</summary>
	protected virtual void Upgrade() { }

	/// <summary>Functionality for derived classes.</summary>
	protected virtual void OnStart() { }

	/// <summary>Update the value of the scroll bar if necessary.</summary>
	...
	/// <summary>Invalidate the scroll bar.</summary>
	...
	/// <summary>Drag the scroll bar by the specified on-screen amount.</summary>
	protected float ScreenToValue(Vector2 screenPos) {
		// Create a plane
		var trans = cachedTransform;
		var plane = new Plane(trans.rotation * Vector3.back, trans.position);
	...
	/// <summary>Calculate the value of the progress bar given the specified local position.</summary>
	protected virtual float LocalToValue(Vector2 localPos) {
		if(mFG != null) {
			...
			if(isHorizontal) {
				var diff = (localPos.x - corners[0].x) / size.x;
				return isInverted ? 1f - diff : diff;
			}
			else {
				var diff = (localPos.y - corners[0].y) / size.y;
				return isInverted ? 1f - diff : diff;
			}
		}
		return value;
	...
	/// <summary>Update the value of the scroll bar.</summary>
	public virtual void ForceUpdate() {
		...
		if(thumb != null && (mFG != null || mBG != null)) {
			var corners = mFG != null ? mFG.localCorners : mBG.localCorners;

			var br = mFG != null ? mFG.border : mBG.border;
			corners[0].x += br.x;
			corners[1].x += br.x;
			corners[2].x -= br.z;
			corners[3].x -= br.z;
			...
			var t = mFG != null ? mFG.cachedTransform : mBG.cachedTransform;
			for(var i = 0; i < 4; ++i) corners[i] = t.TransformPoint(corners[i]);

			if(isHorizontal) {
				var v0 = Vector3.Lerp(corners[0], corners[1], 0.5f);
				var v1 = Vector3.Lerp(corners[2], corners[3], 0.5f);
				SetThumbPosition(Vector3.Lerp(v0, v1, isInverted ? 1f - value : value));
			}
			else {
				var v0 = Vector3.Lerp(corners[0], corners[3], 0.5f);
				var v1 = Vector3.Lerp(corners[1], corners[2], 0.5f);
				SetThumbPosition(Vector3.Lerp(v0, v1, isInverted ? 1f - value : value));
			}
		}
	...
	/// <summary>Set the position of the thumb to the specified world coordinates.</summary>
	...
	/// <summary>Watch for key events and adjust the value accordingly.</summary>
	public virtual void OnPan(Vector2 delta) {
		if(enabled)
			switch(mFill) {
				case FillDirection.LeftToRight: {
					var after = Mathf.Clamp01(mValue + delta.x);
					value = after;
					mValue = after;
					break;
				}
				...
				case FillDirection.BottomToTop: {
					var after = Mathf.Clamp01(mValue + delta.y);
					value = after;
					mValue = after;
					break;
				}
