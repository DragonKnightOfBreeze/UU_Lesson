...
/// <summary>Extended progress bar that has backwards compatibility logic and adds interaction support.</summary>
[ExecuteInEditMode]
...
public class UISlider : UIProgressBar {
	...
	[HideInInspector]
	[SerializeField]
	private float rawValue = 1f; // Use 'value'
	[HideInInspector]
	[SerializeField]
	private Direction direction = Direction.Upgraded; // Use 'fillDirection'
	[HideInInspector]
	[SerializeField]
...
#else
		var bg = mBG != null && (mBG.GetComponent<Collider>() != null || mBG.GetComponent<Collider2D>() != null) ? mBG.gameObject : gameObject;
		var bgl = UIEventListener.Get(bg);
		bgl.onPress += OnPressBackground;
		bgl.onDrag += OnDragBackground;
...
#endif 
		{
			var fgl = UIEventListener.Get(thumb.gameObject);
			fgl.onPress += OnPressForeground;
			fgl.onDrag += OnDragForeground;
		}
	}

	/// <summary>Position the scroll bar to be under the current touch.</summary>
	...
	/// <summary>Save the position of the foreground on press.</summary>
	...
	/// <summary>Drag the scroll bar in the specified direction.</summary>
	...
	public override void OnPan(Vector2 delta) {
		if(enabled && isColliderEnabled) base.OnPan(delta);
	}
}