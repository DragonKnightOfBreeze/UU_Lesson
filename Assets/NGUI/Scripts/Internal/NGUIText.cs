...
/// <summary>Helper class containing functionality related to using dynamic fonts.</summary>
public static class NGUIText {
	...
	/// <summary>
	///     When printing text, a lot of additional data must be passed in. In order to save allocations, this data is not
	///     passed at all, but is rather set in a single place before calling the functions that use it.
	/// </summary>
	...
	/// <summary>Recalculate the 'final' values.</summary>
	...
	/// <summary>Prepare to use the specified text.</summary>
	...
	/// <summary>Get the specified symbol.</summary>
	public static BMSymbol GetSymbol(string text, int index, int textLength) {
		return bitmapFont != null ? bitmapFont.MatchSymbol(text, index, textLength) : null;
	}

	/// <summary>Get the width of the specified glyph. Returns zero if the glyph could not be retrieved.</summary>
	public static float GetGlyphWidth(int ch, int prev, float fontScale) {
		if(bitmapFont != null) {
			...
			var bmg = bitmapFont.bmFont.GetGlyph(ch);
	...
	/// <summary>Get the specified glyph.</summary>
	...
	private static readonly Color mInvisible = new Color(0f, 0f, 0f, 0f);
	private static readonly BetterList<Color> mColors = new BetterList<Color>();
	private static float mAlpha = 1f;
#if DYNAMIC_FONT
	private static CharacterInfo mTempChar;
#endif

	/// <summary>Parse Aa syntax alpha encoded in the string.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	public static float ParseAlpha(string text, int index) {
		var a = (NGUIMath.HexToDecimal(text[index + 1]) << 4) | NGUIMath.HexToDecimal(text[index + 2]);
		return Mathf.Clamp01(a / 255f);
	}

	/// <summary>Parse a RrGgBb color encoded in the string.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	...
	/// <summary>Parse a RrGgBbAa color encoded in the string.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	public static Color ParseColor32(string text, int offset) {
		...
		var g = (NGUIMath.HexToDecimal(text[offset + 2]) << 4) | NGUIMath.HexToDecimal(text[offset + 3]);
		var b = (NGUIMath.HexToDecimal(text[offset + 4]) << 4) | NGUIMath.HexToDecimal(text[offset + 5]);
		var a = (NGUIMath.HexToDecimal(text[offset + 6]) << 4) | NGUIMath.HexToDecimal(text[offset + 7]);
		var f = 1f / 255f;
		return new Color(f * r, f * g, f * b, f * a);
	}

	/// <summary>The reverse of ParseColor -- encodes a color in RrGgBb format.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	...
	/// <summary>Convenience function that wraps the specified text block in a color tag.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	...
	/// <summary>The reverse of ParseAlpha -- encodes a color in Aa format.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	public static string EncodeAlpha(float a) {
		var i = Mathf.Clamp(Mathf.RoundToInt(a * 255f), 0, 255);
		return NGUIMath.DecimalToHex8(i);
	}

	/// <summary>The reverse of ParseColor24 -- encodes a color in RrGgBb format.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	...
	/// <summary>The reverse of ParseColor32 -- encodes a color in RrGgBb format.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	public static string EncodeColor32(Color c) {
		var i = NGUIMath.ColorToInt(c);
		return NGUIMath.DecimalToHex32(i);
	}

	/// <summary>
	///     Parse an embedded symbol, such as [FFAA00] (set color) or [-] (undo color change). Returns whether the index
	///     was adjusted.
	/// </summary>
	...
	/// <summary>Whether the specified character falls under the 'hex' character category (0-9, A-F).</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	...
	/// <summary>
	///     Parse the symbol, if possible. Returns 'true' if the 'index' was adjusted. Advanced symbol support originally
	///     contributed by Rudy Pangestu.
	/// </summary>
	public static bool ParseSymbol(string text, ref int index, BetterList<Color> colors, bool premultiply,
		...
		if(text[index + 2] == ']') {
			...
			var sub3 = text.Substring(index, 3);
		...
		if(text[index + 3] == ']') {
			...
			switch(sub4) {
				...
				default: {
					var ch0 = text[index + 1];
					var ch1 = text[index + 2];
		...
		if(text[index + 4] == ']') {
			var sub5 = text.Substring(index, 5);
		...
		if(text[index + 1] == 'u' && text[index + 2] == 'r' && text[index + 3] == 'l' && text[index + 4] == '=') {
			var closingBracket = text.IndexOf(']', index + 4);
		...
		if(text[index + 7] == ']') {
			var c = ParseColor24(text, index + 1);
		...
		if(text[index + 9] == ']') {
			var c = ParseColor32(text, index + 1);
	...
	/// <summary>Runs through the specified string and removes all color-encoding symbols.</summary>
	...
	/// <summary>Align the vertices to be right or center-aligned given the line width specified by NGUIText.lineWidth.</summary>
	public static void Align(List<Vector3> verts, int indexOffset, float printedWidth, int elements = 4) {
		switch(alignment) {
			case Alignment.Right: {
				var padding = rectWidth - printedWidth;
			...
			case Alignment.Center: {
				var padding = (rectWidth - printedWidth) * 0.5f;
				if(padding < 0f) return;

				// Keep it pixel-perfect
				var diff = Mathf.RoundToInt(rectWidth - printedWidth);
				var intWidth = Mathf.RoundToInt(rectWidth);
			...
			case Alignment.Justified: {
				...
				var chars = (verts.Count - indexOffset) / elements;
				if(chars < 1) return;

				var progressPerChar = 1f / (chars - 1);
				var scale = rectWidth / printedWidth;
				Vector3 v;

				for(int i = indexOffset + elements, charIndex = 1, imax = verts.Count; i < imax; ++charIndex) {
					var x0 = verts[i].x;
					var x1 = verts[i + elements / 2].x;
					var w = x1 - x0;
					var x0a = x0 * scale;
					var x1a = x0a + w;
					var x1b = x1 * scale;
	...
	/// <summary>
	///     Get the index of the closest character within the provided list of values. Meant to be used with the arrays
	///     created by PrintExactCharacterPositions().
	/// </summary>
	...
	/// <summary>
	///     Get the index of the closest vertex within the provided list of values. This function first sorts by Y, and
	///     only then by X. Meant to be used with the arrays created by PrintApproximateCharacterPositions().
	/// </summary>
	public static int GetApproximateCharacterIndex(List<Vector3> verts, List<int> indices, Vector2 pos) {
		...
		for(int i = 0, imax = verts.Count; i < imax; ++i) {
			var diffY = Mathf.Abs(pos.y - verts[i].y);
			if(diffY > bestY) continue;

			var diffX = Mathf.Abs(pos.x - verts[i].x);
	...
	/// <summary>Whether the specified character is a space.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	...
	/// <summary>
	///     Convenience function that ends the line by either appending a new line character or replacing a space with
	///     one.
	/// </summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	...
	/// <summary>Convenience function that ends the line by replacing a space with a newline character.</summary>
	[DebuggerHidden]
	[DebuggerStepThrough]
	...
	/// <summary>Get the printed size of the specified string. The returned value is in pixels.</summary>
	...
	private static readonly BetterList<float> mSizes = new BetterList<float>();

	/// <summary>Calculate the character index offset required to print the end of the specified text.</summary>
	public static int CalculateOffsetToFit(string text) {
		...
		for(int i = 0, imax = text.Length; i < imax; ++i) {
			...
			if(symbol == null) {
				ch = text[i];
				var w = GetGlyphWidth(ch, prev, scale);
		...
		var currentCharacterIndex = mSizes.size;
	...
	/// <summary>Get the end of line that would fit into a field of given width.</summary>
	public static string GetEndOfLineThatFits(string text) {
		var textLength = text.Length;
		var offset = CalculateOffsetToFit(text);
		return text.Substring(offset, textLength - offset);
	}

	/// <summary>Text wrapping functionality. The 'width' and 'height' should be in pixels.</summary>
	...
	/// <summary>
	///     Text wrapping functionality. The 'width' and 'height' should be in pixels. Returns 'true' if the requested
	///     text fits into the previously set dimensions.
	/// </summary>
	public static bool WrapText(string text, out string finalText, bool keepCharCount, bool wrapLineColors, bool useEllipsis = false) {
		...
		var height = maxLines > 0 ? Mathf.Min(regionHeight, finalLineHeight * maxLines) : regionHeight;
		...
		var strikethrough = false;
		var ignoreColor = false;
		var ellipsisWidth = useEllipsis ? (finalSpacingX + GetGlyphWidth('.', '.', fontScale)) * 3f : finalSpacingX;
		var lastValidChar = 0;
		...
		for(; offset < textLength; ++offset) {
			...
			if(ch == '\n') {
				...
				if(wrapLineColors) {
					...
					for(var i = 0; i < mColors.size; ++i) {
						sb.Append("[");
						sb.Append(EncodeColor(mColors[i]));
						sb.Append("]");
					}
				}
			...
			if(symbol == null) {
				...
				glyphWidth = finalSpacingX + symbol.advance * scale;
			}
	...
	private static Color s_c0, s_c1;
	private const float sizeShrinkage = 0.75f;

	/// <summary>Print the specified text into the buffers.</summary>
	public static void Print(string text, List<Vector3> verts, List<Vector2> uvs, List<Color> cols) {
		...
		var gb = tint * gradientBottom;
		var gt = tint * gradientTop;
		var gc = tint;
		var textLength = text.Length;

		var uvRect = new Rect();
		float invX = 0f, invY = 0f;
		var sizePD = sizeF * pixelDensity;
		...
		for(var i = 0; i < textLength; ++i) {
			...
			if(symbol != null) {
				...
				if(cols != null) {
					if(symbolStyle == SymbolStyle.Colored) {
						for(var b = 0; b < 4; ++b) cols.Add(gc);
					}
					else {
						var col = Color.white;
						col.a = gc.a;
						for(var b = 0; b < 4; ++b) cols.Add(col);
					}
				}
			}
			else // No symbol present
			{
				...
				if(!bold) {
					...
					{
						var slant = fontSize * 0.1f * ((v1y - v0y) / fontSize);
				...
				{
					for(var j = 0; j < 4; ++j) {
						var a = mBoldOffset[j * 2];
						var b = mBoldOffset[j * 2 + 1];

						var slant = italic ? fontSize * 0.1f * ((v1y - v0y) / fontSize) : 0f;
				...
				if(underline || strikethrough) {
					...
					if(uvs != null) {
						...
						var cx = (dash.u0.x + dash.u2.x) * 0.5f;
					...
					if(gradient) {
						var min = sizePD + dash.v0.y / scale;
						var max = sizePD + dash.v1.y / scale;
	...
	private static readonly float[] mBoldOffset = {
		                                              -0.25f, 0f, 0.25f, 0f,
		                                              0f, -0.25f, 0f, 0.25f
	                                              };

	/// <summary>
	///     Print character positions and indices into the specified buffer. Meant to be used with the "find closest
	///     vertex" calculations.
	/// </summary>
	public static void PrintApproximateCharacterPositions(string text, List<Vector3> verts, List<int> indices) {
		...
		for(var i = 0; i < textLength; ++i) {
			...
			if(symbol == null) {
				...
				var w = symbol.advance * scale + finalSpacingX;
	...
	/// <summary>
	///     Print character positions and indices into the specified buffer. This function's data is meant to be used for
	///     precise character selection, such as clicking on a link. There are 2 vertices for every index: Bottom Left + Top
	///     Right.
	/// </summary>
	public static void PrintExactCharacterPositions(string text, List<Vector3> verts, List<int> indices) {
		...
		for(var i = 0; i < textLength; ++i) {
			...
			if(symbol == null) {
				var gw = GetGlyphWidth(ch, prev, scale);

				if(gw != 0f) {
					var w = gw + finalSpacingX;
	...
	/// <summary>
	///     Print the caret and selection vertices. Note that it's expected that 'text' has been stripped clean of
	///     symbols.
	/// </summary>
	public static void PrintCaretAndSelection(string text, int start, int end, List<Vector3> caret, List<Vector3> highlight) {
		...
		var caretOffset = caret != null ? caret.Count : 0;
		var highlightOffset = highlight != null ? highlight.Count : 0;
		...
		var last0 = Vector2.zero;
		var last1 = Vector2.zero;
		...
		for(; index < textLength; ++index) {
			...
			var w = symbol != null ? symbol.advance * scale : GetGlyphWidth(ch, prev, scale);
	...
	/// <summary>Replace the specified link.</summary>
	public static bool ReplaceLink(ref string text, ref int index, string prefix) {
		if(index == -1) return false;
		index = text.IndexOf(prefix, index);
		if(index == -1) return false;

		var domainStart = index + prefix.Length;
		var end = text.IndexOf(' ', domainStart);
		if(end == -1) end = text.Length;

		var domainEnd = text.IndexOfAny(new[] {'/', ' '}, domainStart);
		...
		var link = text.Substring(index, end - index);
		var right = text.Substring(end);
		var urlName = text.Substring(domainStart, domainEnd - domainStart);
	...
	/// <summary>Insert a hyperlink around the specified keyword.</summary>
	public static bool InsertHyperlink(ref string text, ref int index, string keyword, string link) {
		var patchStart = text.IndexOf(keyword, index, System.StringComparison.CurrentCultureIgnoreCase);
		if(patchStart == -1) return false;

		var left = text.Substring(0, patchStart);
		var url = "[url=" + link + "][u]";
		var middle = text.Substring(patchStart, keyword.Length) + "[/u][/url]";
		var right = text.Substring(patchStart + keyword.Length);
	...
	/// <summary>Helper function that replaces links within text with clickable ones.</summary>
	public static void ReplaceLinks(ref string text) {
		for(var index = 0; index < text.Length;)
			if(!ReplaceLink(ref text, ref index, "http://"))
				break;

		for(var index = 0; index < text.Length;)
			if(!ReplaceLink(ref text, ref index, "https://"))
				break;
	}
}